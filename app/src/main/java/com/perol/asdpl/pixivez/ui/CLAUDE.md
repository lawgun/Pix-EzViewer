...
| `MainActivity.kt` | 底部导航宿主。按 `main_mode` pref 装配 `HelloMainViewPager`(插画)或 `NovelMainViewPager`(小说);抽屉项 `nav_novel_mode` 翻转 pref 后 `finish()+startActivity` 全新实例重装(recreate 会按 tag 复用旧模式 fragment,禁用)。 |
